...
export function RecommendationPage() {
  ...
  const generateInitialRecommendations = () => {
    // Generate initial recommendations based on common travel interests
    ...
    setMlRecommendations(initialRecommendations);
  ...
  const fetchNearbyPlaces = async (lat: number, lon: number) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        const nearbyPlaces: Place[] = data.map((place: any, index: number) => {
          ...
          return {
            ...
            image: `https://source.unsplash.com/random/800x600/?${encodeURIComponent(place.display_name.split(',')[0])}`,
